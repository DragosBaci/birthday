export const AppConfig = {
  celebrantName: "Numele Meu", // REPLACE WITH REAL NAME
  eventDate: "2025-02-21T19:00:00", // ISO format for potential countdown
  dateDisplay: "21 februarie",
  timeDisplay: "19:00",
  locationName: "Ted Bistro Reghin",
  addressLink: "https://www.google.com/maps/place/Bistro+TED/@46.77702,24.6960915,17z/data=!3m2!4b1!5s0x474a3797feadf009:0xcf61af34dcfdcf1a!4m6!3m5!1s0x474a3745aff7e587:0x460a5706ea637a52!8m2!3d46.7770164!4d24.6986664!16s%2Fg%2F11r742jrbj?entry=ttu&g_ep=EgoyMDI2MDIxMS4wIKXMDSoASAFQAw%3D%3D", // REPLACE WITH REAL LINK
  rsvpContact: "07xx xxx xxx", // REPLACE WITH REAL CONTACT
  dressedCode: "Smart Casual / Party Vibes",
  extraJokeLine: "Dacă nu vii, plânge tortul.",
  
  // Scene content
  intro: {
    title: "PREGĂTEȘTE-TE!",
    subtitle: "Urmează cea mai tare petrecere din an...",
    button: "DA-I DRUMUL!"
  },
  teaser: {
    lines: [
      "Nu e o glumă.",
      "Chiar se întâmplă.",
      "Și o să fie...",
      "LEGEN- (așteaptă) -DAR!"
    ],
    button: "ZI-MI MAI MULTE"
  },
  details: {
    title: "DETALIILE CARE CONTEAZĂ",
    copyButton: "COPIAZĂ DETALII"
  },
  photo: {
    title: "POZA MEA DE LEGENDĂ",
  },
  rsvp: {
    question: "VII LA PETRECERE?",
    yesButton: "DA! 100%",
    noButton: "NU...", // This text is critical for the logic
    successTitle: "YEEEEES!",
    successMessage: "Abia aștept să ne distrăm!",
    addToCalendar: "ADAUGĂ ÎN CALENDAR",
    sendMail: "TRIMITE CONFIRMARE"
  },
  final: {
    title: "NE VEDEM ACOLO!",
    shareLink: "COPIAZĂ LINKUL",
    subtitle: "Nu uita să aduci voia bună!"
  }
};
